const CART_KEY = "meaty_cart";
const CUSTOMER_KEY = "meaty_customer";
const WHATSAPP_NUMBER = "918078937260";
...
const renderCart = () => {
  ...
  if (!cart.length) {
    cartItems.innerHTML = '<p class="text-sm text-stone-500">Your cart is empty. Add products to get started.</p>';
    return;
  }
    ...
    .map(
      ...
      <div class="mb-3 rounded-lg border border-stone-200 p-3">
        <div class="flex items-start gap-3">
          <img src="${item.image}" alt="${item.name}" class="h-14 w-14 rounded-md border border-stone-200 object-cover p-1" />
          <div class="flex-1">
            <p class="text-sm font-bold text-stone-900">${item.name} ${item.unit ? `(${item.unit})` : ""}</p>
            <p class="text-xs text-stone-500">${formatMoney(item.price)} each</p>
...
const buildWhatsappMessage = () => {
  ...
  cart.forEach((item, index) => {
    ...
    message += `${index + 1}. ${nameWithUnit} - ${item.qty} x \u20B9${item.price} = \u20B9${lineTotal}%0A`;
  });
...
const buildWhatsappPlainText = () => {
  ...
  cart.forEach((item, index) => {
    const lineTotal = item.price * item.qty;
    const nameWithUnit = item.unit ? `${item.name} (${item.unit})` : item.name;
    lines.push(`${index + 1}. ${nameWithUnit} - ${item.qty} x \u20B9${item.price} = \u20B9${lineTotal}`);
  });
...
document.addEventListener("click", (e) => {
  ...
  if (addBtn instanceof HTMLElement) {
    ...
    const image = addBtn.dataset.productImage || "/assets/images/oyster.jpg";
    const unit = addBtn.dataset.productUnit || "";
    ...
    addToCart({ slug, name, price, image, unit }, qty);
    return;
  }
